...
def list_tags_of_resource(ResourceArn, region=None, key=None, keyid=None, profile=None):
    ...
    '''
    conn3 = __utils__['boto3.get_connection']('dynamodb', region=region, key=key, keyid=keyid,
            profile=profile)
    retries = 10
    sleep = 6
    Tags = []
    while retries:
        try:
            ...
            while Marker is not None:
                ret = conn3.list_tags_of_resource(ResourceArn=ResourceArn, NextToken=Marker)
                Tags += ret.get('Tags', [])
            ...
            if retries and err.response.get('Error', {}).get('Code') == 'Throttling':
                ...
                time.sleep(sleep)
                continue
            log.error('Failed to list Tags for resource %s: %s', ResourceArn, err.message)
            return None
...
def untag_resource(ResourceArn, TagKeys, region=None, key=None, keyid=None, profile=None):
    '''
    ...
    while retries:
        try:
            ...
            if retries and err.response.get('Error', {}).get('Code') == 'Throttling':
                ...
                time.sleep(sleep)
                continue
            log.error('Failed to remove Tags from resource %s: %s', ResourceArn, err.message)
            return False
